...
void func65()
{
	size_t res = 0, answer = 272;

	// e = [2; 1, 2, 1, 1, 4, 1, 1, 6, 1, ..., 1, 2k, 1, ...]
	auto getNthNum = [](size_t n) -> size_t
	{
		size_t res = 1;
	...
	auto add_and_inverse = [](stringNum &nom, stringNum &denom, size_t n) -> void
	{
		nom = nom + denom * n;
		stringNum N = denom;
		denom = nom;
	...
	start -= 2;
	stringNum nom   = 1;
	stringNum denom = getNthNum(start);

	for (size_t i = start; i > 0; i--)
	{
		add_and_inverse(nom, denom, getNthNum(i - 1));
	}

	add_and_inverse(nom, denom, 2);
	add_and_inverse(nom, denom, 0);

	std::cout << " --   nom : " <<   nom.get() << std::endl;
	std::cout << " -- denom : " << denom.get() << std::endl;
...
void func66()
{
	...
	auto getSequence = [&](size_t N, std::vector<size_t>& vec) -> void
	{
		size_t a = 1, b = 0, a_old = -1, b_old = -1;
	...
	auto getConvergents = [](size_t i, const std::vector<size_t> &vecA, std::vector<stringNum> &vecP, std::vector<stringNum> &vecQ)
	{
		if (i == 0)
		{
			vecP.clear();
		...
		const size_t *ptr = (i < vecA.size()) ? &vecA[i] : &vecA[i%vecA.size() + 1];

		switch (i)
		{
			case 0:
				vecP.push_back(vecA[0]);
				vecQ.push_back(1);
				break;

			case 1:
				vecP.push_back(vecA[0] * vecA[1] + 1);
				vecQ.push_back(vecA[1]);
				break;

			default:
				vecP.push_back(stringNum(*ptr) * vecP[i-1] + vecP[i-2]);
				vecQ.push_back(stringNum(*ptr) * vecQ[i-1] + vecQ[i-2]);
		}
	};

	// x^2 - D * y^2 = 1
	auto getSolution = [&](stringNum &X, stringNum &Y, size_t D) -> void
	{
		std::vector<size_t>		vec;
		std::vector<stringNum>	vecP, vecQ;

		getSequence(D, vec);

		for(size_t i = 0; i < size_t(-1); i++)
		{
			getConvergents(i, vec, vecP, vecQ);

			stringNum *x = &vecP.back();
			stringNum *y = &vecQ.back();
			...
			else
			{
				for (numType rem = 0; rem < N; rem++)
				{
					...
					else
					{
						...
						{
							total += func(rem, num, map_1, map_2, mtx1, mtx2);
						}
					}
...
#if 1
	{
		answer = testLesser(3333);
	}
#endif

	validateResult(answer, 0);
}
